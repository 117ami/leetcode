#include "aux.cpp"
/**
In a row of trees, the i-th tree producesfruit with typetree[i].
...
tree:you must perform step 1, then step 2, then back to step 1, then step 2, and
...
public:
  int totalFruit(vector<int> &tree) {
    ...
    int res = 0, ta = tree[0], tb = -1, len = 0, lastidxa = 0, lastidxb = -1;
    for (int i = 0; i < tree.size(); i++) {
      if (tree[i] == ta) {
        ...
        len = i - min(lastidxa, lastidxb);
        res = max(res, len);
        ta = lastidxa > lastidxb ? ta : tb;
        lastidxa = lastidxa > lastidxb ? lastidxa : lastidxb;
        tb = tree[i];
        lastidxb = i;
      }
    }
...
int main() {
  Solution s;
  std::vector<int> tree = {3, 3, 3, 1, 2, 1, 1, 2, 3, 3, 4};
  tree = {0, 1, 2, 2};
  say(s.totalFruit(tree));
}
