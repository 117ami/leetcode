...
public:
  string optimalDivision(vector<int> &nums) {
    string res;
    if (nums.size() < 3)
      return join(nums, 0, nums.size(), "/");
    return to_string(nums[0]) + "/" + "(" + join(nums, 1, nums.size(), "/") +
           ")";
  }

  string join(vector<int> &arr, int start, int end, string seprator) {
    stringstream ss;
