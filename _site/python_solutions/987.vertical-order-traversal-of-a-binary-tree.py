...
class Solution:
    def verticalTraversal(self, r):
        res = []

        def dfs(n, x, y):
            if n:
                res.append([n.val, x, y])
                dfs(n.left, x - 1, y - 1)
                dfs(n.right, x + 1, y - 1)
        dfs(r, 0, 0)
        res.sort(key=lambda p: (p[1], -1 * p[2], p[0]))
        x = -1 * float('inf')
        t = []
        for p in res:
            if p[1] != x:
        ...
        # values = set(map(lambda x: x[1], res))
        # res = [[y for y in res if y[1] == x] for x in values]
        return t


# arr = [3, 9, 20, None, None, 15, 7]
# arr = [1, 2, 3, 4, 5, 6, 7]
# r = aux.tree_from_list(arr)
# s = Solution()
