...
def find_poisoned_duration(time_series, duration)
  res = 0
  return duration * time_series.size if time_series.size <= 1
  0.upto(time_series.size - 2).each do |i|
    xtime = time_series[i + 1] - time_series[i]
    res += xtime >= duration ? duration : xtime
  end
  res + duration
end
...
time_series = [1, 8]
duration = 2
p find_poisoned_duration(time_series, duration)