...
(3) allows deletion in logarithmic time. 
"""
...
class Node(object):
    ...
    def add(self, key, value):
        ...
        # If new key is larger than all existing keys, insert new key to the
        # right of all existing keys.
        if i == len(self.keys):
            self.keys.append(key)
            self.values.append([value])
        ...
        # If new key is smaller than existing key, insert new key to the left
        # of existing key.
    ...
    def split(self):
        ...
        left, right = Node(self.order), Node(self.order)
        mid = self.order // 2

        left.keys, right.keys = self.keys[:mid], self.keys[mid:]
        left.values, right.values = self.values[:mid], self.values[mid:]

        # When the node is split, set the parent key to the left-most key of
...
class BPlusTree(object):
    ...
    Nodes will automatically be split into two once it is full. When a split occurs,
    ...
    def _find(self, node, key):
        ...
        # Not in this level, then in child level
    ...
    def insert(self, key, value):
        """Inserts a key-value pair after traversing to a leaf node. If the 
        leaf node is full, split the leaf node into two.
        """
        parent = None
        child = self.root
