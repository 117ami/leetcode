...
def video_stitching(clips, t)
  ...
  i = count = max_end = 0
  loop do
    return -1 if i >= clips.size || clips[i].first > max_end

    new_end = max_end
    while i < clips.size && clips[i].first <= max_end
      new_end = [new_end, clips[i].last].max
      i += 1
    end
    max_end = new_end
    count += 1
    return count if max_end >= t
  end
  -1
end
