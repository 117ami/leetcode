...
public:
    TreeNode* constructFromPrePost(vector<int>& pre, vector<int>& post) {
      ...
      int idx = 0; 
      while (post[idx] != pre[1]) idx++; 
      vi preleft(pre.begin() + 1, pre.begin() + idx + 2), preright(pre.begin() + idx + 2, pre.end());
      vi postleft(post.begin(), post.begin() + idx + 1), postright(post.begin() + idx +1, post.end()-1);
