...
class Solution:
    def rankTeams(self, votes: List[str]) -> str:
        # print(list(zip(*votes)))
        cnt = [Counter(p) for p in zip(*votes)]
        return ''.join(sorted(sorted(votes[0]), key=lambda e: [c[e] for c in cnt], reverse=True))
        # print(cnt)
        ...
        #         r[ord(c) - 65][i] += 1
        # r = [(i, l) for i, l in enumerate(r)]
        # r.sort(key=lambda e: (e[1], -e[0]))
        # # print(r)
        # s = [chr(i + 65) for i, vec in r if sum(vec) > 0]
        # return ''.join(s[::-1])
