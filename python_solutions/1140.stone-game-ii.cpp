...
public:
    ...
    int dp(int i, int k, vi& piles, vi& vsum){
        int key = i * 1000 + k; 
        if (exist(memo, key)) return memo[key];
        if (i + k * 2 >= piles.size()) return vsum[i];
        int res = 0; 
        forup(l, 1, 2*k + 1){
          int knext = max(k, l);
          res = max(res, vsum[i] - dp(i + l, knext, piles, vsum));
        }
        memo[key] = res; 
        return res; 
    }
    ...
    int stoneGameII(vector<int>& piles) {
      int n = piles.size(); 
      vi vsum(piles); 
      fordown(i, 0, n - 1) vsum[i] += vsum[i + 1];
      return dp(0, 1, piles, vsum);
    }
};
