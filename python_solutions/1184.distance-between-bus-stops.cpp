...
class Solution {
public:
    int distanceBetweenBusStops(vector<int>& ds, int s, int e) {
        if (s > e) return distanceBetweenBusStops(ds, e, s); 
        if (s == e) return 0; 
        int sum_val = qsum(ds), part = 0; 
        forup(i, s, e) part += ds[i]; 
        return min(part, sum_val - part);
    }
};
