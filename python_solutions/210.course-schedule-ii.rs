 ...
 */
impl Solution {
    pub fn kahn(graph: Vec<Vec<i32>>, ref mut indegree: Vec<i32>) -> Vec<i32> {
        let n = graph.len();
        let mut sorted: Vec<i32> = vec![];
        let mut sources = (0..n as i32)
            .filter(|&i| indegree[i as usize] == 0)
            .collect::<VecDeque<i32>>();

        while !sources.is_empty() {
            let u = sources.pop_back().unwrap();
            sorted.push(u);
    ...
    pub fn find_order(num_courses: i32, prerequisites: Vec<Vec<i32>>) -> Vec<i32> {
        ...
        for t in prerequisites {
            graph[t[0] as usize].push(t[1]);
            indegree[t[1] as usize] += 1;
        }
        Self::kahn(graph, indegree)
    }
}
