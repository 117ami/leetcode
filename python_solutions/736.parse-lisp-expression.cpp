...
public:
    ...
    int eval(string e, unordered_map<string, int> & map) {
      if (e[0] != '(') {
        if (e[0] == '-' || ('0' <= e[0] && e[0] <= '9')) return stoi(e);
        return map[e];
      }

      unordered_map<string, int> thismap(map); 
      int sid = e[1] == 'm' ? 6 : 5; 
      vector<string> tokens = parse(e.substr(sid, e.size() - sid - 1));
      if (e[1] == 'a') {
        return eval(tokens[0], thismap) + eval(tokens[1], thismap); 
      } else if (e[1] == 'm') {
        return eval(tokens[0], thismap) * eval(tokens[1], thismap); 
      } else {
        for(int i = 0; i < tokens.size() - 2; i+=2) {
          thismap[tokens[i]] = eval(tokens[i+1], thismap);
        }
        return eval(tokens.back(), thismap)        ;
      }
    }

    vector<string> parse(string e) {
      vector<string> res; 
      int cter = 0; 
      string t; 
      for(auto c: e){
        if (c == '(') cter ++; 
        if (c == ')') cter --; 
        if (cter == 0 and c == ' ') res.push_back(t), t = "";
        else t.push_back(c);
      }
