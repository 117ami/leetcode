...
public:
  int minCost(vector<vector<int>> &grid) {
    ...
    deque<vector<int>> q = {{m - 1, n - 1, 0}};
    vi dirs = {-1, 0, 1, 0, -1}, signs = {3, 2, 4, 1};

    while (!q.empty()) {
      vi x = q.front();
      // say(x);
      int i = x[0], j = x[1], cost = x[2];
      q.pop_front();
      if (seen[i][j])
        continue;
      seen[i][j] = true;
      ...
      for (auto k : {0,3,1,2}) {
        ...
        if (ni < 0 || nj < 0 || ni >= m || nj >= n)
          continue;
        if (grid[ni][nj] == signs[k])
          q.push_front(vi{ni, nj, cost});
        else
          q.push_back(vi{ni, nj, cost + 1});
      }
    }
    return -1;
  }
