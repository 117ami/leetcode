...
public:
    ...
    TreeNode* helper(TreeNode* root, bool is_root) {
      if (!root) return root; 
      bool root_removed = exist(m, root->val);
      if (is_root && !root_removed) 
        res.eb(root); 
      root->left = helper(root->left, root_removed); 
      root->right = helper(root->right, root_removed);       
      return root_removed ? NULL : root; 
    }
};
