...
#
class Solution:
    def distanceBetweenBusStops(self, ds, s, e):
        if s > e: return self.distanceBetweenBusStops(ds, e, s)
        if s == e: return 0
        return min(sum(ds[0:s] + ds[e:]), sum(ds[s:e]))

# s = Solution()
# ds = [1,2,3,4]
# st, e = 1, 3
# print(s.distanceBetweenBusStops(ds, st, e) )
