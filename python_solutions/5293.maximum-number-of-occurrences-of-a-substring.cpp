...
/*
 * @lc app=leetcode id=5293 lang=cpp
 *
...
public:
    int maxFreq(string s, int maxLetters, int minSize, int maxSize) {
        ...
        int start = 0, intRes = 0; 
        fori(i, s.size()) {
          char c = s[i], c_st = s[start];
          cter[c] ++; 
          if (i - start + 1 == minSize){
            if (cter.size() <= maxLetters) {
              string tmp = s.substr(start, i - start + 1);
              res[tmp]++; 
              intRes = max(intRes, res[tmp]); 
            }
            cter[c_st] --; 
            if (cter[c_st] == 0) cter.erase(c_st); 
            start ++; 
          }
        }
        // for(auto &[key, value]: res) cout << key << " " << value << endl; 
        return intRes;
    }
};
