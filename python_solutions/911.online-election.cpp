...
public:
    ...
    TopVotedCandidate(vector<int>& persons, vector<int>& _times): times(_times), votes() {
        int curmax = 0; 
        umii cter; 
        for(auto p: persons){
          if (++cter[p] >= curmax) curmax = cter[p], votes.pb(p); 
          else votes.pb(votes.back());
        }
    };
    
    int q(int t) {
        int i = bisect_right(times, t);
        if (i == 0) return votes.front(); 
        return votes[i-1];
    }
};
