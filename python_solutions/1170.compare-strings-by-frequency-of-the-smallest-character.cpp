...
public:
    ...
    vector<int> numSmallerByFrequency(vector<string>& queries, vector<string>& words) {
     vi res; 
     vi q = {}, w = {}; 
     transform(queries.begin(), queries.end(), back_inserter(q), f);
     transform(words.begin(), words.end(), back_inserter(w), f);     
     dsort(w);
     
     each(i, q.size() - 1)
       res.push_back(w.size() - bisect_right(w, q[i])); 
