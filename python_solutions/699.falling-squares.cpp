...
public:
    vector<int> fallingSquares(vector<vector<int>>& positions) {
      ...
      for(auto &p: positions) {
        ...
        int premax_h = it == mp.begin() ? 0 : prev(it)->second; 
        int tmp_h = premax_h; 

        while (it != mp.end() && it->first < end){
          premax_h = max(premax_h, it->second);
          tmp_h = it->second; 
          it = mp.erase(it);
          // say(vi{premax_h, tmp_h});
        }

        maxh = max(maxh, premax_h + h);
        mp[start] = premax_h + h; 
        mp[end] = tmp_h; 
        res.push_back(maxh);
      }
    ...
    vector<int> fallingSquares_2(vector<vector<int>>& positions) {
        vector<int> res = {};
        vector<Interval> intervals; 
        int h = 0, max_h = 0; 
        for(auto &p: positions){
          Interval it(p[0], p[0] + p[1], p[1]);
          h = getMaxHeight(intervals, it);
          max_h = max(max_h, h);
          res.push_back(max_h);
        }
        return res; 
    ...
    int getMaxHeight(vector<Interval> &intervals, Interval &it){
      int premax = 0; 
      for(auto &i: intervals){
        if(i.end <= it.start || it.end <= i.start) continue; 
        premax = max(premax, i.height);
      }
      it.height += premax; 
