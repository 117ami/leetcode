...
class Solution {
public:
  bool sorted(string s, string t, unordered_map<char, int> &cc) {
    forloop(i, s.size()) {
      if (i == t.size() || cc[t[i]] < cc[s[i]])
        return false;
      if (cc[s[i]] < cc[t[i]])
        return true;
    }
  ...
  bool isAlienSorted(vector<string> &words, string order) {
    ...
    forloop(i, order.size()) cc[order[i]] = i;
    forloop(i, words.size() -1) 
      if (!sorted(words[i], words[i + 1], cc)) return false;
    return true;
  }
};
