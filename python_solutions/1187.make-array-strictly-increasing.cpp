    ...
    int makeArrayIncreasing(vector<int>& a, vector<int>& b) {
      ...
      for(auto i: a){
        ...
        for(const auto &[k, v]: dp){
          if (i > k) tmp[i] = exist(tmp, i) ? min(tmp[i], v) : v;
          int loc = bisect_right(b, k) ; 
          if (loc < b.size()) tmp[b[loc]] = exist(tmp, b[loc]) ? min(tmp[b[loc]], 1+v) :1+v;
        }
        dp = tmp; 
      }
      if (dp.empty()) return -1; 
      int res = 0x3f3f3f3f; 
      for(const auto &[k, v]: dp) res = min(res, v);
      return res; 
    }
};
