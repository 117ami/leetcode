...
public:
  bool circularArrayLoop(vector<int> &nums) {
    ...
    int bound = (5000 / nums.size() + 1) * nums.size();
    for (int j = 0; j < nums.size(); j ++) {
      int i = j; 
      if (nums[i] >= 1001) continue; 
      int mark = bound + i, val = nums[i]; 
      while (nums[i] < 1001 && nums[i] * val > 0 && (nums[i] + nums.size()) % nums.size() != 0) {
        int jump = nums[i];
        nums[i] = mark; 
        i = (i + jump + nums.size()) % nums.size();
      }
  ...
  bool circularArrayLoop_2(vector<int> &nums) {
    int size = nums.size();
...
private:
  int getIndex(int i, vector<int> &nums) {
    return i + nums[i] >= 0 ? (i + nums[i]) % nums.size()
                            : (i + nums[i] + nums.size()) % nums.size();
  }
};
