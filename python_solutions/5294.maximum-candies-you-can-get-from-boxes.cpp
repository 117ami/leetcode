...
/*
 * @lc app=leetcode id=5294 lang=cpp
 *
...
public:
    int maxCandies(vector<int>& status, vector<int>& candies, vector<vector<int>>& keys, vector<vector<int>>& containedBoxes, vector<int>& initialBoxes) {
        ...
        queue<int> jobs; 
        for(auto i: initialBoxes) jobs.push(i);
        int res = 0; 
        unordered_set<int> seen, locked, keys_local;

        while(!jobs.empty()) {
            int c = jobs.front(); 
            jobs.pop(); 
            if (exist(seen, c)) continue; 
            seen.insert(c);
            res += candies[c];

            for (auto n: keys[c]) keys_local.insert(n);
            vi tmp(containedBoxes[c]);
            for (auto k: locked) tmp.pb(k);
            
            for(auto n: tmp) {
              if (status[n] == 1 || exist(keys_local, n)) jobs.push(n); 
              else locked.insert(n);
            }
        }
        return res; 
    }
