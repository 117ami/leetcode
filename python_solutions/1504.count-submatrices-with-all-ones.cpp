...
class Solution {
public:
  int _calculate_pre_sum(vector<int> &h) {
    int n = qsize(h);
    vi _sum(n, 0);
    stack<int> st;
    forloop(i, n) {
      while (!st.empty() && h[st.top()] >= h[i])
        st.pop();
      if (!st.empty()) {
        int _preindex = st.top();
        _sum[i] = _sum[_preindex];
        _sum[i] += h[i] * (i - _preindex);
      } else {
        _sum[i] = h[i] * (i + 1);
      }
      st.push(i);
  ...
  int numSubmat(vector<vector<int>> &mat) {
    int m = qsize(mat), n = qsize(mat[0]), ans = 0;
    vi h(n, 0);
    forloop(i, m) {
      forloop(j, n) h[j] = mat[i][j] == 0 ? 0 : h[j] + 1;
      ans += _calculate_pre_sum(h);
    }
    return ans;
  }
};
