...
public:
    string longestCommonPrefix(vector<string>& strs) {
      ...
      dsort(strs);
      if (strs.front() == strs.back()) return strs.back(); 
      int i = 1; 
      while(i <= strs.front().size()){
        if (strs.front().substr(0, i) == strs.back().substr(0, i)) i ++; 
        else return strs.front().substr(0, i-1);
      }
      return strs.front().substr(0, i-1) == strs.back().substr(0, i-1) ? strs.back().substr(0, i-1):"";
    }
};
