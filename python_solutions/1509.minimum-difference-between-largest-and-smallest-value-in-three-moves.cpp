...
public:
  int minDifference(vector<int> &nums) {
    ...
    if (n <= 4)
      return 0;
    int ans = INT_MAX;
    // shift the largest 4 numbers back, and smallest 4 numbers front
    std::partial_sort(nums.begin(), nums.begin() + 4, nums.end());
    std::partial_sort(nums.rbegin(), nums.rbegin() + 4, nums.rend(),
                      std::greater<int>());
    forloop(i, 4) { ans = min(ans, nums[n - 4 + i] - nums[i]); }
    return ans;
  }
};
