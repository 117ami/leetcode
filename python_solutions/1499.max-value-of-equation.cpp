...
public:
    int findMaxValueOfEquation(vector<vector<int>>& points, int k) {
        ...
        int ans  = INT_MIN; 
        for(auto &p: points)         {
          while (!d.empty() && d.front().first + k < p[0]) d.pop_front(); 
          if (!d.empty()) ans = max(ans, p[1] + d.front().second + p[0]); 
          while(!d.empty() && d.back().second <= p[1] - p[0]) d.pop_back(); 
          d.push_back({p[0], p[1]-p[0]});
          // say(ans);
        }
        return ans ;
    }
