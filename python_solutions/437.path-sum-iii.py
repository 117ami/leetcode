...
class Solution:
    def secPathSum(self, r, sum):
        if not r:
            return 0
        res = 1 if r.val == sum else 0
        return res + self.secPathSum(r.left, sum - r.val) + self.secPathSum(r.right, sum - r.val) 
        
    def pathSum(self, root, sum):
        if not root: return 0
        return self.secPathSum(root, sum) + self.pathSum(root.left, sum) + self.pathSum(root.right, sum)


s = Solution()
arr = [10,5,-3,3,2,None,11,3,-2,None,1]
arr = [1,-2,-3,1,3,-2,None,-1]
# arr = [5,3,2,3,-2,None, 1]
r = tree_from_list(arr)
# print(s.secPathSum(r, 8))
print(s.pathSum(r, 1))
