...
#define down(i, b, a)                                                          \
  for (size_t i = int(b); i >= int(a); --i) // reverse [a, b)
...
public:
  vector<vector<int>> reconstructMatrix(int upper, int lower,
                                        vector<int> &colsum) {
    size_t n = colsum.size();
    vvi res(2, vi(n, 0));
    each(i, n - 1) if (colsum[i] == 2) res[0][i] = res[1][i] = 1, lower--,
                                       upper--;
    if (lower < 0 || upper < 0)
      return vvi{};

    each(i, n - 1) if (colsum[i] == 1) {
      if (upper > 0)
        res[0][i] = 1, upper--;
      else
        res[1][i] = 1, lower--;
