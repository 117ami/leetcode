...
class Solution {
public:
  bool similar(string &sa, string &sb) {
    int cnt = 0;
    forloop(i, sa.size()) {
      if (sa[i] != sb[i] && ++cnt > 2)
        return false;
  ...
  int numSimilarGroups(vector<string> &A) {
    ...
    if (word_len > 6) {
      for (auto &word : sa) {
        int step = word_len / 3;
        for (int i = 0; i < word_len; i += step) {
          parts[word.substr(i, step) + itos(i)].pb(word);
        }
      }
    } else {
      parts["all"] = A;
    ...
    for (auto &[k, list] : parts) {
      forloop(i, list.size()) {
        string sa = list[i];
        forloopup(j, i + 1, list.size()) {
          string sb = list[j];
          if (similar(sa, sb)) {
            graph[sa].insert(sb);
            graph[sb].insert(sa);
  ...
  //     }
  //   }
  //   forloop(i, n) uf.parent[i] = uf.find(i);
  //   unordered_set<int> ans(uf.parent.begin(), uf.parent.end());
  //   // say(ans);
  //   return ans.size();
  // }
};
