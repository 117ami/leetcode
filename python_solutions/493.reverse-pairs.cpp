...
public:
  int mergesort(vector<int> &nums, int lo, int hi){
      if (lo >= hi) return  0; 
      int cnt = 0, mid = (lo + hi) >> 1; 
      cnt += mergesort(nums, lo, mid) + mergesort(nums, mid + 1, hi);
      for (int i = lo, j = mid + 1; j <= hi; ++j) {
        while (i <= mid && nums[i] <= 2ll * nums[j]) ++ i; 
        cnt += mid + 1 - i; 
      }
      for (int i = lo; i <= hi; i ++) aux[i] = nums[i];
      int li = lo, ri = mid + 1, ci = lo; 
      while (li <= mid && ri <= hi){
        if (aux[li] <= aux[ri]) nums[ci ++] = aux[li++];
        else nums[ci ++ ] = aux[ri++];
      }

      while (li <= mid) nums[ci ++] = aux[li++];
      while (ri <= hi) nums[ci ++] = aux[ri++];
