...
public:
  vector<vector<string>> suggestedProducts(vector<string> &products,
    ...
    size_t lo = 0, hi = products.size() - 1;
    for (size_t i = 0; i < sw.size(); ++i) {
      char c = sw[i];
      while (lo <= hi && (products[lo].size() <= i || products[lo][i] != c))
        lo++;
      while (lo <= hi && (products[hi].size() <= i || products[hi][i] != c))
        hi--;
      if (lo > hi)
        res.push_back({});
      else {
        res.push_back(vector<string>(products.begin() + lo,
                                     products.begin() + min(lo + 3, hi + 1)));
      }
    }
    return res;
  }
