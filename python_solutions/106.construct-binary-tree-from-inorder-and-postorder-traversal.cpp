...
class Solution {
public:
    TreeNode* rec(int lo, int hi, umii &idx, vi& ino, deque<int>& pos) {
      if (lo > hi) return nullptr; 
      int mid = idx[pos.back()]; 
      TreeNode* r = new TreeNode(pos.back());
      pos.pop_back();
      r->right = rec(mid + 1, hi, idx, ino, pos);
      r->left = rec(lo, mid - 1, idx, ino, pos);
      return r; 
    }

    TreeNode* buildTree(vector<int>& inorder, vector<int>& postorder) {
      umii idx; 
      qfor(i, qsize(inorder)) idx[inorder[i]] = i; 
      deque<int> pos (qall(postorder));
      return rec(0, inorder.size() - 1, idx, inorder, pos);
    }
};
