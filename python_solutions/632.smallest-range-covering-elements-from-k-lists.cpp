...
void sort_by_last(vvi &a) {
  sort(a.begin(), a.end(),
       [](const auto &p, const auto &q) { return p.back() < q.back(); });
}
...
public:
    vector<int> smallestRange(vector<vector<int>>& nums) {
      ...
      fori(i, k) for(auto n: nums[i]) vv.pb(pair<int, int>(i, n)); 
      sort(vv.begin(), vv.end(), [](const auto&p, const auto&q){return p.second < q.second;});

      int i = 0, nmax = 0x3f3f3f3f; 
      bool allseen = false; 
      unordered_map<int, int> memo; 
      vi res; 

      fori(j, vv.size()){
        int arrid = vv[j].first, n = vv[j].second; 
        ...
        while (i < j && vv[i].first == -1) i++; 
        if (!allseen) allseen = (memo.size() == k); 

        // say(vi{nmax, int(allseen), n, vv[i][1]});
        if (allseen && n - vv[i].second < nmax){
          nmax = n - vv[i].second; 
          res = {vv[i].second, n};
        }
      }
