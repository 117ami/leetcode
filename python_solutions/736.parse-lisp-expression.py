...
class Solution:
    ...
    def eval(self, expr, map):
        ...
        map_curr = map.copy()
        tokens = self.parse(expr[6:-1] if expr[1] == 'm' else expr[5:-1])
        if expr.startswith('(a'):  # add
            return self.eval(tokens[0], map_curr) + self.eval(tokens[1], map_curr)
        elif expr.startswith('(m'):  # Multiplication
            return self.eval(tokens[0], map_curr) * self.eval(tokens[1], map_curr)
        else:
            for i in range(0, len(tokens) - 2, 2):
                map_curr[tokens[i]] = self.eval(tokens[i + 1], map_curr)
            return self.eval(tokens[-1], map_curr)
...
# expr = "(let x 3 (let x 4 x))"
# expr = "(let a1 3 b2 (add a1 1) b2)"
print(s.evaluate(expr))