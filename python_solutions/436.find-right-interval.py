...
class Solution:
    def findRightInterval(self, intervals: List[List[int]]) -> List[int]:
        sp = [(i, p[0]) for i, p in enumerate(intervals)]
        sp.sort(key=lambda e: e[1])
        i32sp = [e[1] for e in sp]
        # print(sp, i32sp)
        res = [0] * len(intervals)

        for i, it in enumerate(intervals):
            j = bisect_left(i32sp, it[1])
            res[i] = sp[j][0] if j < len(intervals) else -1
        return res
...
its = [[1, 4], [2, 3], [3, 4]]
its = [ [3,4], [2,3], [1,2] ]
print(sol.findRightInterval(its))
