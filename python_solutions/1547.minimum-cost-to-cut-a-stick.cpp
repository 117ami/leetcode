...
public:
  int minCost(int n, vector<int> &cuts) {
    ...
    qsort(cuts);
    int size = qsize(cuts);
    vvi dp(size, vi(size, INT_MAX));

    forloopup(d, 1, size) forloop(l, size - d) if (d == 1) dp[l][l + d] = 0;
    else {
      forloopup(j, l + 1, l + d) dp[l][l + d] =
          min(dp[l][l + d], cuts[l + d] - cuts[l] + dp[l][j] + dp[j][l + d]);
    }
    return dp[0].back();
  }
};
