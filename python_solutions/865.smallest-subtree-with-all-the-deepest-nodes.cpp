...
public:
    pair<TreeNode*, int> dfs(TreeNode* n){
      if (n == nullptr) return mp(nullptr, 0);
      auto left = dfs(n->left), right = dfs(n->right); 
      if (left.second > right.second) return mp(left.first, left.second+1);
      else if (left.second < right.second) return mp(right.first, right.second+1);
      else  return mp(n, right.second+1);
    }
