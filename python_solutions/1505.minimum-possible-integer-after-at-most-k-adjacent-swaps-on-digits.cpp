...
public:
  ...
  int solve(vector<int> &ns, int k, int d) {
    if (k <= 0)
      return k;
    std::vector<int> foo(ns.size()), cc;
    std::iota(foo.begin(), foo.end(), 0);
    bool valid = true;

    std::copy_if(foo.begin(), foo.end(), std::back_inserter(cc),
                 [&ns, &valid, d](int i) {
                   return ns[i] == d &&
                          (valid &= (i == 0 || (i > 0 && ns[i] <= ns[i - 1])));
                 });

    for (int i = 0; i < cc.size() && cc[i] - i <= k; i++) {
      k -= cc[i] - i;
      pre.emplace_back(d), ns[cc[i]] = -1;
    }

    // Remove negative placeholder -1
  ...
  string minInteger(string num, int k) {
    vector<int> tmp; 
    std::transform(num.begin(), num.end(), std::back_inserter(tmp), [](char c){return c - '0';});
    
    forloop(i, 10) k = solve(tmp, k, i);
    
    // say(k);say(tmp);say(pre);
    size_t prelen = pre.size(), i = 0;
    pre.insert(pre.end(), tmp.begin(), tmp.end());
    if (!tmp.empty())
      while (i < k)
        swap(pre[prelen - i], pre[prelen - i - 1]), i++;

    stringstream result;
    copy(pre.begin(), pre.end(), ostream_iterator<int>(result, ""));
    return result.str();
  }
};
