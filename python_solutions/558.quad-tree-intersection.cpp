...
public:
    Node* intersect(Node* p, Node* q) {
        ...
        Node* tl = intersect(p->topLeft, q->topLeft); 
        Node* tr = intersect(p->topRight, q->topRight);
        Node* bl = intersect(p->bottomLeft, q->bottomLeft); 
        Node* br = intersect(p->bottomRight, q->bottomRight);
        bool v = tl->val; 
        if (tl->isLeaf && tr->isLeaf && bl->isLeaf && br->isLeaf && tl->val == v && tr->val == v && bl->val == v && br->val == v){
          return new Node(v, true, nullptr, nullptr, nullptr, nullptr); 
        } else 
        return  new Node(v, false, tl, tr, bl, br); 
    }
};
