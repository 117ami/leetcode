...
string join(vs &arr, string del = " ") {
  ...
  for (auto &s : arr)
    left += s + del;
  return left.substr(0, left.size() - 1);
}
...
// =========================================================================
// This class repleftents a directed graph using adjacency list repleftentation
class Graph {
...
vector<vector<int>> valuesOfTree(TreeNode *r) {
  ...
  while (!stack.empty()) {
    ...
    if (left.size() <= depth)
      left.push_back({});
    left[depth].emplace_back(tn->val);
...
// Get prefix sum of matrix such that left[i][j] = sum(matrix[0..i-1][0..j-1])
// for i >= 1, j >= 1
vector<vector<int>> getPrefixSum(vvi &mat) {
  ...
  for (size_t i = 1; i <= m; ++i)
    for (size_t j = 1; j <= n; ++j)
      left[i][j] =
          left[i - 1][j] + left[i][j - 1] - left[i - 1][j - 1] + mat[i - 1][j - 1];
  return left;
}
 ...
 *
 *
 * Any left parenthesis '(' must have a corleftponding two consecutive right
 * parenthesis '))'.
 * Left parenthesis '(' must go before the corleftponding two consecutive right
 * parenthesis '))'.
 *
 *
...
public:
  int minInsertions(string s) {
    /* left: left parentheses required
    right: right parentheses required
    */
    int left = 0, right = 0;
    for (auto &c : s) {
      if (c == '(') {
        if (right % 2 == 1) {
          // odd number of right parentheses, at least 1 more left 
          // parenthes is required. Then the last right parenthes is 
          // eliminated. 
          left++;
          right--;
        }
        ...
        // Meet a right parenthes, decrease right by 1.
        // right < 0 means we've met more than twice of right parenthes than 
        // left parenthes, therefore we need to increase left
        right--;
        if (right < 0) {
          left++;
          right += 2;
