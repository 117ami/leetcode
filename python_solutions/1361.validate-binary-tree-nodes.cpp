...
public:
    bool validateBinaryTreeNodes(int n, vector<int>& lc, vector<int>& rc) {
        ...
        for(int i = 0; i < rc.size(); i++) children.push_back(make_pair(lc[i], rc[i]));
        stack<int> jobs; jobs.push(0);

        while (!jobs.empty()) {
          int cur = jobs.top(); jobs.pop(); 
          if (seen.find(cur) != seen.end()) return false; 
          seen.insert(cur);

          pair<int, int> pi = children[cur];
          if (pi.first >= 0) jobs.push(pi.first);
          if (pi.second >= 0) jobs.push(pi.second);
        }
