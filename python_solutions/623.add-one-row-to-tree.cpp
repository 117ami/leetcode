...
public:
    TreeNode* addOneRow(TreeNode* root, int v, int d) {
      ...
      queue<pair<int, TreeNode*>> jobs; 
      jobs.push(pair<int, TreeNode*>{1, root}); 
      while(! jobs.empty()) {
        int dcur = jobs.front().first; 
        TreeNode* r = jobs.front().second; 
        jobs.pop(); 
        if (!r) continue; 
        if (dcur == d - 1) {
          ...
          jobs.push(pair<int, TreeNode*>{dcur+1, r->left});
          jobs.push(pair<int, TreeNode*>{dcur+1, r->right});
        }
      }
      return root; 
    }
