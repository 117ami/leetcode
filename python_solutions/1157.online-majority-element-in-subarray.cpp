...
public:
    ...
    MajorityChecker(vector<int>& arr) {
        forloop(i, qsize(arr)) loc[arr[i]].pb(i);
        for(auto &[k, v]: loc) ns.pb(k);
        sort(ns.begin(), ns.end(), [=](const int &a, const int &b){ return qsize(loc[a]) > qsize(loc[b]);});
    }
    
    int query(int left, int right, int threshold) {
        for(auto n: ns) {
          if (qsize(loc[n]) < threshold) return -1; 
          int l = bisect_left(loc[n], left); 
          int r = bisect_right(loc[n], right);
