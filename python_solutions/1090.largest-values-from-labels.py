...
class Solution:
    def largestValsFromLabels(self, v, l, m, mx):
        a = sorted(zip(v, l), reverse=True)
        visited = {}
        res = 0
        for iv, il in a:
            if il not in visited or visited[il] < mx:
                res += iv
                visited[il] = visited.get(il, 0) + 1
                m -= 1
            if m == 0:
                return res
        return res
...
s = Solution()
v, l, m, mx = [8, 8, 7, 6, 9], [0, 0, 1, 1, 0], 3, 2
print(s.largestValsFromLabels(v, l, m, mx))  # = 2
