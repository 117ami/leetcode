...
class Solution:
    def maximumGap(self, nums):
        ...
        gap = (maxv - minv) // (n - 1) + 1
        bucket_min = [Numbers().INF] * (n - 1)
        bucket_max = [Numbers().NEGINF] * (n - 1)
        ...
        maxgap, premax = Numbers().NEGINF, minv
        for i in range(n - 1):
            if bucket_min[i] == Numbers(
            ).INF and bucket_max[i] == Numbers().NEGINF:
                continue
            maxgap = max(maxgap, bucket_min[i] - premax)
            premax = bucket_max[i]
