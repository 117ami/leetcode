...
class Solution {
public:
    vector<int> filterRestaurants(vector<vector<int>>& rest, int vf, int mp, int md) {
        vvi ans; 
        for(auto &v: rest) if (v[2] >= vf && v[3] <= mp && v[4] <= md) ans.pb(v);
        sort(ans.begin(), ans.end(), [](const vector<int> &a, const vector<int> &b){ return a[1] > b[1] || (a[1] == b[1] && a[0] > b[0]);} );
        vi res; 
        for(auto &v: ans) res.pb(v[0]);
        return res; 
    }
};
