...
public:
    int n; 
    umii memo; 
    int recur(int i, vi& days, vi& costs){
      if (exist(memo, i)) return memo[i]; 
      if (i >= n) return 0; 
      if (i == n - 1) return vecmin(costs);

      // int j = upper_bound(days.begin(), days.end(), days[i] + 29) - days.begin(); 
      int j = bisect_right(days, days[i] + 29);
      int c3 = costs[2] + recur(j, days, costs);

      // j = upper_bound(days.begin(), days.end(), days[i] + 6) - days.begin(); 
      j = bisect_right(days, days[i] + 6);
      int c2 = costs[1] + recur(j, days, costs);

      int c1 = costs[0] + recur(i + 1, days, costs);
