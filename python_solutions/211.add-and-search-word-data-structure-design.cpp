...
public:
    ...
    unordered_map<int, vector<string>> dd; 
    WordDictionary() {
    ...
    void addWord(string word) {
        dd[qsize(word)].pb(word);
        cc.insert(word);
    }
    
    /** Returns if the word is in the data structure. A word could contain the dot character '.' to represent any one letter. */
    bool search(string word) {
        if (exist(cc, word)) return true; 
        for (auto &p: dd[qsize(word)]) {
          bool flag = true; 
          forloop(i, qsize(word)) {
            if (word[i] != '.' and word[i] != p[i]) { 
              flag = ! flag; 
              break; 
            }
          }
