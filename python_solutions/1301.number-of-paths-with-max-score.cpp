...
public:
    vector<int> pathsWithMaxScore(vector<string>& board) {
      ...
      vector<int> res; 
      fori(i, n) fori(j, n){
        if (board[i][j] == 'X') dp[i][j] = mp(-10000, 1);
        else if (i == 0 && j == 0) dp[i][j] = mp(0, 1);
        else if (i == 0) dp[i][j] = {dp[i][j-1].first + board[i][j] - '0', 1};
        else if (j == 0) dp[i][j] = {dp[i-1][j].first + board[i][j] - '0', 1};
        else {
          pair<long, long> cur = dp[i][j], p1 = dp[i-1][j], p2 = dp[i][j-1], p3 = dp[i-1][j-1];
          long vmax = max(max(p1.first, p2.first), p3.first); 
          if (vmax == p1.first) cur.second += p1.second; 
          if (vmax == p2.first) cur.second += p2.second; 
          if (vmax == p3.first) cur.second += p3.second; 
          cur.first = board[i][j] == 'S' ? 0 : (board[i][j] - '0'); 
          cur.first += vmax;
          cur.second %= MOD; 
          dp[i][j] = mp(cur.first, cur.second);
        }
      }
      pair<long, long> ans  = dp.back().back(); 
      // say(ans.first); 
      // say(ans.second);
      return ans.first >= 0 ? vi{int(ans.first), int(ans.second)} : vi{0, 0};
      // return res; 
    }
};
