...
public:
    int ans = 0; 
    vector<int> rec(TreeNode* node, int d) {
      if(!node) return {0};
      if(!node->left && !node->right) return {1};
      auto l = rec(node->left, d); 
      auto r = rec(node->right, d); 
      for(auto x: l) for(auto y: r) if (x && y && (x+y<=d)) ans ++; 
      vi res; 
      for(auto x: l) if (x && x+1<d) res.pb(x+1);
      for(auto x: r) if (x && x+1<d) res.pb(x+1);
      return res; 
    }
