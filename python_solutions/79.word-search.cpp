...
public:
  bool dfs(vector<vector<char>> &board, int i, int j, int k, string &word,
    ...
    if (k == qsize(word))
      return true;
    if (i < 0 || j < 0 || i >= qsize(board) || j >= qsize(board[0]) ||
        seen[i][j] || word[k] != board[i][j])
      return false;
    char c = board[i][j];
    board[i][j] = '$';
    seen[i][j] = 1;
    if (dfs(board, i + 1, j, k + 1, word, seen))
      return true;
    if (dfs(board, i - 1, j, k + 1, word, seen))
      return true;
    if (dfs(board, i, j + 1, k + 1, word, seen))
      return true;
    if (dfs(board, i, j - 1, k + 1, word, seen))
      return true;
    board[i][j] = c;
  ...
  bool exist(vector<vector<char>> &board, string word) {
    if (qsize(board) == 0)
      return false;
    vvi seen(qsize(board), vi(qsize(board[0]), 0));
    forloop(i, qsize(board)) forloop(
        j, qsize(board[0])) if (dfs(board, i, j, 0, word, seen)) return true;
    return false;
  }
};
