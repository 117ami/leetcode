...
public:
    ...
    int dfs(TreeNode* node){
      ...
      int lc = dfs(node->left), rc = dfs(node->right);
      cter[node->val] = lc + 1 + rc; 
      if (node->val == target) l = lc, r = rc; 
      return cter[node->val];
    }
    bool btreeGameWinningMove(TreeNode* root, int n, int x) {
        target = x; 
        dfs(root);
        return cter[root->val] > cter[x] * 2 || max(l, r) * 2 > cter[root->val];
    }
};
