...
public:
  ...
  double getMinDistSum(vector<vector<int>> &positions) {
    ...
    double err = 1e-7, epsilon = 1e-20;
    pair<double, double> pre = {100.0, 100.0}, cur = {0.0, 0.0};

    for (auto &p : ps)
      cur.first += p.first / ps.size(), cur.second += p.second / ps.size();

    while (dist(cur, pre) > err) {
      double f = 0, s = 0, d = 0;
      for (auto &p : ps) {
        double l2 = dist(p, cur) + epsilon;
        f += p.first / l2, s += p.second / l2, d += 1 / l2;
      }
      pre = cur, cur = {f / d, s / d};
    }

    vector<double> ds(ps.size(), 0);
    transform(
        ps.begin(), ps.end(), ds.begin(),
        [&cur, this](pair<double, double> &b) { return this->dist(cur, b); });
    return accumulate(ds.begin(), ds.end(), 0.0);
  }
};
