...
#define fori(i, n) for (size_t i = 0; i < n; i++) //[0, n)
#define forir(i, n) for (int i = n - 1; i >= 0; i--)   // reverse [0, n)
#define forup(i, a, b) for (size_t i = a; i < b; ++i) // [a, b)
#define fordown(i, a, b) for (size_t i = b - 1; i >= a; --i) // reverse [a, b)
...
bool is_prime(ll x) {
  ...
  forup(i, 2, m) { if (!(x % i)) return false; }
  return true;
}
