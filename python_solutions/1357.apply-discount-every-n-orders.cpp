...
class Cashier {
public:
    int cter = 0, total = 0, dis = 0; 
    unordered_map<int, int> ps; 
    Cashier(int n, int discount, vector<int>& products, vector<int>& prices) {
        total = n, dis = discount; 
        for(int i = 0; i < products.size(); i++) ps[products[i]] = prices[i];
    }
    
    double getBill(vector<int> product, vector<int> amount) {
        double res = 0; 
        for(int i = 0; i < product.size(); i++) res += (double)(ps[product[i]] * amount[i]); 
        if (++ cter == total) res -= res * (double(dis) / 100), cter = 0; 
        return res; 
    }
};
