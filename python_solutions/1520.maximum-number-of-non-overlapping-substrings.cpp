...
class Solution {
public:
  // Change char to usize.
  int c2u(char c) { return c - 'a'; }

  int _search_new_right(string &s, int i, vector<int> &l, vector<int> &r) {
    int right = r[c2u(s[i])];
    for (auto j = i; j <= right; ++j) {
      if (l[c2u(s[j])] < i)
        return -1;
      right = max(right, r[c2u(s[j])]);
    }
    return right;
  }

  vector<string> maxNumOfSubstrings(string s) {
    vi l(26, INT_MAX), r(26, 0);
    int n = qsize(s);
    forloop(i, n) {
      int idx = c2u(s[i]);
      l[idx] = min(l[idx], i);
    ...
    forloop(i, n) {
      ...
      int idx = l[c2u(c)];
      if (i == idx) {
        int _right = _search_new_right(s, i, l, r);
        if (_right > -1) {
          if (i > right)
            res.pb("");
          right = _right;
