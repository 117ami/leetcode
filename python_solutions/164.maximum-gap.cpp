...
public:
    int maximumGap(vector<int>& nums) {
      int n = nums.size(); 
      if (n < 2)   return 0; 
      int maxv = vec_max(nums), minv = vec_min(nums); 
      int gap = (maxv - minv) / (n - 1) + 1; 
      vi bucket_min(n - 1, INT_MAX), bucket_max(n - 1, INT_MIN); 

      for (auto &i: nums) {
        if (i == maxv || i == minv) continue; 
        int idx = (i - minv) / gap; 
        bucket_min[idx] = min(i, bucket_min[idx]); 
        bucket_max[idx] = max(i, bucket_max[idx]); 
      }
      
      int res = INT_MIN, premax = minv; 
      for (int i = 0; i < n - 1; i ++){
        if (bucket_max[i] == INT_MIN && bucket_min[i] == INT_MAX) continue; 
        res = max(res, bucket_min[i] - premax); 
        premax = bucket_max[i] ; 
      }
      res = max(res, maxv - premax); 
      return res; 
    }
};
