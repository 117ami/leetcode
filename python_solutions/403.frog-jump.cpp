...
public:
  bool canCross(vector<int> &stones) {
    forup(i, 3, stones.size()) if (stones[i] > stones[i - 1] * 2) return false;

    unordered_set<int> set(dall(stones));
    stack<int> poses, jumps;
    poses.push(0), jumps.push(0);
    while (!poses.empty()) {
      int p = poses.top(), j = jumps.top();
      poses.pop(), jumps.pop();
      forup(i, j - 1, j + 2) {
        if (i < 1)
          continue;
