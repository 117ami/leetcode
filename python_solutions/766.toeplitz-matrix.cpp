...
class Solution {
public:
    bool toe(int i, int j, vvi& mx) {
      while (i < mx.size() - 1 && j < mx[0].size() - 1) {
        if (mx[i][j] != mx[++i][++j]) return false; 
      }
      return true; 
    }

    bool isToeplitzMatrix(vector<vector<int>>& mx) {
      fori(i, mx.size()) if (! toe(i, 0, mx)) return false;
      fori(i, mx[0].size()) if (!toe(0, i, mx)) return false; 
      return true; 
    }
};
